    ...
    case ArrayK:
        {
            ...
            return new arrayIR(s,menType2TypePtr(rt->attr.ArrayAttr.memtype),rt->attr.ArrayAttr.low);
        }
    ...
    case IdK:
        {
            ...
            else {semanticsError("Undeclared type!",rt->lineno);return &intir;}
        }
        ...
        case ArrayK:
            for(int i=0;i<rt->name.size();++i)
                if(rt->attr.ArrayAttr.memtype==IntegerK)
                    fprintf(cppfile,"int _%s[%d]%s",rt->name[i].c_str(),rt->attr.ArrayAttr.up - rt->attr.ArrayAttr.low + 1,sep.c_str());
                else
                    fprintf(cppfile,"char _%s[%d]%s",rt->name[i].c_str(),rt->attr.ArrayAttr.up - rt->attr.ArrayAttr.low + 1,sep.c_str());
            break;
        case IdK:
            for(int i=0;i<rt->name.size();++i)
...
typeIR * getExpType(TreeNode * rt,AccessKinds * ac,typeIR * recmemptr)
{
    switch(rt->kind.exp)
    {
        case ConstK:
                        ...
                        case FieldMembV:
                            {
                                ...
                                if(pos != -1)
                                {
                                    ...
                                    {semanticsError("Undeclared record member!",rt->lineno);return &intir;}
                            }
                            break;
                    }
                }
                else {semanticsError("Undeclared variable!",rt->lineno);return &intir;}
            }
        ...
        case OpK:
            ...
            if(gext0 != &intir && gext0 != &charir)
                {semanticsError("Cannot operate Array or Record!",rt->lineno);return gext0;}
            if(gext0 == gext1)
            {
            ...
            else {semanticsError("Types not match between Left part and Right part of the expression!",rt->lineno);return &intir;}
    }
}
...
void  StmLKWrite(TreeNode * rt)
{
    ...
    if(gext == &intir || gext == &charir ||(gext->kind == arrayTy &&((arrayIR*)gext)->elemTyPtr == &charir))
        ...
        semanticsError("Cannot write Integer Array or Record!",rt->lineno);
}
void  StmLKRead(TreeNode * rt)
{
    ...
    else
    {
        ...
        if((sn->attrIR.typePtr->kind == arrayTy && air->elemTyPtr==&intir) || sn->attrIR.typePtr->kind == recordTy)
            semanticsError("Cannot read Integer Array or Record!",rt->lineno);
    }
}
...
void  StmLKAssign(TreeNode * rt)
{
    ...
    if(gext1 != gext2 || ac0 ==indir) {semanticsError("Assigning types not match!",rt->lineno);return;}
    if(gext1 != &charir && gext1 != &intir) semanticsError("Cannot Assign Array or Record!",rt->lineno);
}
